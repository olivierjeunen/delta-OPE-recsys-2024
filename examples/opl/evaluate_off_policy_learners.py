...
if __name__ == "__main__":
    ...
    policy_values_opt_baseline, grad_vars_opt_baseline = train_ips_optimal(n_epochs, bandit_train_loader, hnet, bandit_feedback_test, dataset, device, opt)
    ...
    f = open(os.path.join(log_path, 'banditnet_hyperparams'), 'rb')
    ips_banditnet_tune = pickle.load(f)
    hnet = PolicyNet(dim_context, n_actions)
    if optimizer == "adam":
        opt = torch.optim.Adam(hnet.parameters(), lr=lr, weight_decay=0.0)
    elif optimizer == "adamW":
        ...
        opt = torch.optim.SGD(hnet.parameters(), lr=lr) 
    best_lambda = lambda_range[np.argmax(ips_banditnet_tune)]
    print("best lambda for banditNet %f"%best_lambda)
    policy_value_banditnet, grad_vars_banditnet = train_ips_banditnet(n_epochs, bandit_train_loader, hnet, bandit_feedback_test, dataset, device, opt, banditnet_lambda=best_lambda )
