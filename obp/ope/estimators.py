...
class InverseProbabilityWeighting(BaseOffPolicyEstimator):
    ...
    def estimate_variance(
        ...
        **kwargs,
        ...
        """Estimate the sample of the policy value.
        ...
        return np.var(estimated_round_rewards)
...
class SelfNormalizedInverseProbabilityWeighting(InverseProbabilityWeighting):
    ...
    def _estimate_round_rewards(
        ...
        **kwargs,
        ...
        iw = action_dist[np.arange(action.shape[0]), action, position] / pscore
        return reward * iw / np.clip(iw.mean(), a_min=1e-10, a_max=np.Inf)
    ...
    def estimate_variance(
        ...
        **kwargs,
        ...
        iw = action_dist[np.arange(action.shape[0]), action, position] / pscore
        snips_estimate = np.mean(reward * iw / np.clip(iw.mean(), a_min=1e-10, a_max=np.Inf))
        snips_var = np.var(reward * iw / np.clip(iw.mean(), a_min=1e-10, a_max=np.Inf))
        return np.sum(((reward*iw) - snips_estimate*(iw))**2) / np.clip((np.sum(iw)**2), a_min=1e-11, a_max=np.Inf)
        #return snips_var
    
    
@dataclass
class InverseProbabilityWeightingOptimalBaseline(InverseProbabilityWeighting):
    """Inverse Probability Weighting (IPW) Estimator with Optimal Baseline Correction.

    Note
    -------
    IPWOB estimates the policy value of evaluation policy :math:`\\pi_e` as

    .. math::

        \\hat{V}_{\\mathrm{SNIPW}} (\\pi_e; \\mathcal{D}) :=
        \\frac{\\mathbb{E}_{n} [w(x_i,a_i) (r_i - b)]}{ \\mathbb{E}_{n} [w(x_i,a_i)]},
    ...
    def _estimate_round_rewards(
        ...
        **kwargs,
        ...
        iw = action_dist[np.arange(action.shape[0]), action, position] / pscore
        optimal_baseline = (np.mean(reward * (iw**2 - iw)))/(np.clip(np.mean(iw**2 - iw), a_min=1e-11, a_max=np.Inf))
        return (reward - optimal_baseline) * iw + optimal_baseline 
